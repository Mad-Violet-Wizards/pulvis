	...
	public:
			...
			: m_MemoryCategory(_mem_category)
		{
			for (size_t i = 0; i < N; ++i)
			{
				...
				m_Buckets[i].m_Memory = engine::experimental::memory::Allocate<std::byte>(_mem_category, _bucket_capacity);
				m_Buckets[i].m_Ledger = engine::experimental::memory::Allocate<std::byte>(_mem_category, _bucket_capacity / sizeof(T));
